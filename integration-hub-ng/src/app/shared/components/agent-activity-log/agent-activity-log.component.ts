...
@Component({
      ...
      <div class="log-container">
        <div 
          *ngFor="let activity of activities()" 
          ...
          [class.success]="activity.status === 'success'"
          [class.warning]="activity.status === 'warning'"
          [class.error]="activity.status === 'error'">
          
          <div class="entry-icon">
            <svg *ngIf="activity.status === 'info'" width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM13 17H11V15H13V17ZM13 13H11V7H13V13Z" fill="currentColor"/>
            </svg>
            <svg *ngIf="activity.status === 'success'" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M13.5 4L6 11.5L2.5 8" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
            <svg *ngIf="activity.status === 'warning'" width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 2L2 22H22L12 2ZM12 16V18M12 10V14" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
            </svg>
            <svg *ngIf="activity.status === 'error'" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div class="entry-content">
            <div class="entry-header">
              <span class="entry-action">{{ activity.action }}</span>
              <span class="entry-time">{{ formatTime(activity.timestamp) }}</span>
            </div>
            <p class="entry-details" *ngIf="activity.details">{{ activity.details }}</p>
            <div class="entry-metadata" *ngIf="activity.metadata">
              <div *ngFor="let item of getMetadataItems(activity.metadata)" class="metadata-item">
  ...
  styles: [`
    ...
    .entry-metadata {
      margin-top: 0.5rem;
      padding-top: 0.5rem;
      border-top: 1px solid var(--log-border, rgba(0, 0, 0, 0.08));
      display: flex;
      flex-direction: column;
      gap: 0.25rem;
    }
...
export class AgentActivityLogComponent {
  @Input() activities = signal<AgentActivity[]>([]);
  ...
  getMetadataItems(metadata: Record<string, any>): Array<{ key: string; value: string }> {
